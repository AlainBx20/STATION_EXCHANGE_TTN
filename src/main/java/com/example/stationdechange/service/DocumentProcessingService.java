    ...
    public void processSuccessfulCusres(String messageId) {
        ...
        for (Imputations imputations : allImputations) {
            try {
                ...
                Imputations toDelete = imputationsRepository.findById(imputations.getIdImp()).orElse(null);
                if (toDelete != null) {
                    imputationsRepository.delete(toDelete);
                } else {
                    System.err.println("Imputations entity already deleted or not found: id=" + imputations.getIdImp());
                }
